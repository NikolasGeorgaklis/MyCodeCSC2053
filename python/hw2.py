...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    return [x for x in alist if x not in range(min, max+1)]
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    return {word: strings.count(word) for word in strings}
...
def friend_list(friend_dictionary):
    ...
    for key, value in friend_dictionary.items():
        if key in friendship_dictionary:
            friendship_dictionary[key].append(value)
        else:
            friendship_dictionary[key] = [value]
        if value in friendship_dictionary:
            friendship_dictionary[value].append(key)
        else:
            friendship_dictionary[value] = [key]

    return friendship_dictionary
